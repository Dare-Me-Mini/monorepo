...
async function main() {
  ...
  const recipient = getArg("recipient") ?? getArg("feeRecipient") ?? process.env.RECIPIENT;
  if (!recipient) throw new Error("Missing --recipient=<address> or RECIPIENT env var");
  ...
  const tx = await bettingHouse.setFeeRecipient(recipient as `0x${string}`);
  const receipt = await tx.wait();
  console.log(`Set feeRecipient to ${recipient}. Tx: ${receipt?.hash}`);
}
